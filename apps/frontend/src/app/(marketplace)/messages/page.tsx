...
export default function MessagesPage() {
  const { sendMessage, typing, typingUsers, lastMessage } = useMessaging();
  ...
  useEffect(() => {
    if (lastMessage) {
      ...
      if (selectedConvo && lastMessage.conversationId === selectedConvo.id) {
        setMessages(prev => [...prev, lastMessage]);
        scrollToBottom();
      }
      
      // Update conversations list with the latest message snippet
      setConversations(prev => prev.map(c => 
        c.id === lastMessage.conversationId 
          ? { ...c, messages: [lastMessage], updatedAt: new Date().toISOString() }
          : c
  ...
  const handleSend = (e: React.FormEvent) => {
    ...
    const recipientId = currentUser?.id === selectedConvo.buyerId ? selectedConvo.sellerId : selectedConvo.buyerId;
    sendMessage(recipientId, content, selectedConvo.productId, selectedConvo.id);
    setContent('');
    typing(recipientId, false);
  };
        ...
        {selectedConvo ? (
           <>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                   ...
                   <div className="flex items-center gap-1.5">
                      <span className={`w-1.5 h-1.5 rounded-full ${typingUsers.has(currentUser?.id === selectedConvo.buyerId ? selectedConvo.sellerId : selectedConvo.buyerId) ? 'bg-green-500 animate-pulse' : 'bg-gray-300'}`} />
                      <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                         {typingUsers.has(currentUser?.id === selectedConvo.buyerId ? selectedConvo.sellerId : selectedConvo.buyerId) ? 'Typing...' : 'Online'}
                      </p>
                   </div>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-6 bg-gray-50/20">
               {msgLoading && <div className="text-center text-[10px] text-gray-400 uppercase font-black tracking-widest">Sycing history...</div>}
               {messages.map((msg, i) => {
                  ...
                  <div className="flex-1 relative">
                    <Input 
                      ...
                      onChange={(e) => {
                         ...
                         const recipientId = currentUser?.id === selectedConvo.buyerId ? selectedConvo.sellerId : selectedConvo.buyerId;
                         typing(recipientId, e.target.value.length > 0);
                      }}
                    />
                    <div className="absolute right-6 top-1/2 -translate-y-1/2 flex gap-4">
                       {/* Add product context button or emoji later */}
                    </div>
                  </div>
                  <Button 
                    type="submit"
